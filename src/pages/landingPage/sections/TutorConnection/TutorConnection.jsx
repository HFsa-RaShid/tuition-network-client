...
const TutorConnection = () => {
  ...
  return (
    <div className="w-full bg-gray-100/90 py-16 flex justify-center">
      <div className="max-w-3xl w-full px-6">
        <h1 className="text-3xl font-semibold text-center text-gray-800 mb-14">
          The ways <span className="text-blue-500">Tutors</span> can connect
          with us
        </h1>
        ...
        <div className="relative border-l-4 border-blue-300 ml-6">
          {steps.map((step, i) => (
            <div key={i} className="mb-14 relative">
              {/* Connector Dot */}
              <div className="absolute -left-[18px] top-0 w-8 h-8 rounded-full bg-blue-500 shadow-md flex items-center justify-center text-white text-lg font-bold">
                {i + 1}
              </div>
              ...
              {i !== steps.length - 1 && (
                <div className="absolute left-[-3px] top-8 h-12 w-8 border-l-2 border-b-2 border-dotted border-blue-300 rounded-bl-lg"></div>
              )}
            </div>
          ))}
        </div>
